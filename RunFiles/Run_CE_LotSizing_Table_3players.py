...
for m in [3]:
    ...
    for n in [20]:
        f_LotSizing = open("CE_LS_table.txt",'a')
        f_LotSizing.write('m ='+str(m)+' players, n = '+str(n)+' items \n\n')
        f_LotSizing.write(' & ins & time & iter & NE? & supp CE & size S & Decrease on Social Welfare by choosing NE instead & \\\ \n')
        f_LotSizing.close()
        ...
        for ins in [0]:
            ...
            if cpu_time_not_dfs>3600 or Profits_SGM==[]:
                f_LotSizing.write(" & "+ str(ins)+ " & "+'tl & '+str(numb_iter)+' &  & '+str(size_supp_ce) +" & "+str(Numb_stra)+' & &  & \\\ \n')
            elif numb_iter>=max_iter:
                 f_LotSizing.write(" & "+ str(ins)+ " & "+"%0.2f" % cpu_time_not_dfs+' & '+str(numb_iter)+' &  & '+str(size_supp_ce)+" & "+str(Numb_stra)+' & & & \\\ \n')
            else:
                ...
                total_size_1_not_dfs= total_size_1_not_dfs+ Numb_stra[2]
                ...
                if path.exists(filename):
                    ...
                    if sum(Profits_SGM)>0 and Profits_ne!=[]:
                        ...
                        if CE_is_NE:
                            f_LotSizing.write(" & "+ str(ins)+ " & "+"%0.2f" % cpu_time_not_dfs+' & '+str(numb_iter)+' & YES & '+str(size_supp_ce) +" & "+str(Numb_stra)+" & "+ "%0.2f" % decrease+' & & \\\ \n')
                            numb_ce_ne_not_dfs = numb_ce_ne_not_dfs+1
                        else:
                            f_LotSizing.write(" & "+ str(ins)+ " & "+"%0.2f" % cpu_time_not_dfs+' & '+str(numb_iter)+' & NO & '+str(size_supp_ce) +" & "+str(Numb_stra)+" & "+"%0.2f" % decrease+' & & \\\ \n')
                    else:
                        decrease = "-" # we do not have a NE to compare
                        if CE_is_NE:
                            f_LotSizing.write(" & "+ str(ins)+ " & "+"%0.2f" % cpu_time_not_dfs+' & '+str(numb_iter)+' & YES & '+str(size_supp_ce) +" & "+str(Numb_stra)+" & "+ decrease+' & & \\\ \n')
                            numb_ce_ne_not_dfs = numb_ce_ne_not_dfs+1
                        else:
                            f_LotSizing.write(" & "+ str(ins)+ " & "+"%0.2f" % cpu_time_not_dfs+' & '+str(numb_iter)+' & NO & '+str(size_supp_ce) +" & "+str(Numb_stra)+" & "+decrease+' & & \\\ \n')
                else:
                    decrease = "-" # we do not have a NE to compare
                    if CE_is_NE:
                        f_LotSizing.write(" & "+ str(ins)+ " & "+"%0.2f" % cpu_time_not_dfs+' & '+str(numb_iter)+' & YES & '+str(size_supp_ce) +" & "+str(Numb_stra)+" & "+ decrease+' & & \\\ \n')
        ...
        f_LotSizing = open("CE_LS_table.txt",'a')
        f_LotSizing.write(' & avg  & '+ "%0.2f" % total_time_not_dfs+' & '+"%0.2f" % total_iter_not_dfs+' & '+str(numb_ce_ne_not_dfs)+' & '+str(total_supp_ce)+" & "+"%0.2f" % total_size_0_not_dfs+' & '+"%0.2f" % total_size_1_not_dfs+' & '+"%0.2f" % total_size_2_not_dfs+' & '+"%0.2f" % decrease_ce+' \\\ \n')
        f_LotSizing.close()
