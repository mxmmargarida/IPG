...
for m in [2]:
    ...
    #for n in [100]:
        ...
        f_LotSizing.write(' & ins & time & iter & N?E & supp CE & size S & Decrease on Social Welfare by choosing NE instead\\\ \n')
        f_LotSizing.close()
        ...
        f_LotSizing = open("CE_LS_table.txt",'a')
        f_LotSizing.write(' & avg  & '+ "%0.2f" % total_time_not_dfs+' & '+"%0.2f" % total_iter_not_dfs+' & '+str(numb_ce_ne_not_dfs)+' & '+str(total_supp_ce)+" & "+"%0.2f" % total_size_0_not_dfs+' & '+"%0.2f" % total_size_1_not_dfs+' & '+"%0.2f" % decrease_ce+' \\\ \n')
        f_LotSizing.close()
