    ...
    #for m in [2]:
        ...
        #for n in [5]:
            ...
            #for ins in [9]:
                ...
                if cpu_time>3600:
                    f_knapsack.write(" & & "+str(ins)+' & '+'tl & '+str(numb_iter)+' & 0 & 0 & '+str([len(S[i]) for i in range(m)])+' & '+str(numb_back) +' &  ')
                else:
                    if numb_iter >= max_iter:
                        f_knapsack.write(" & & "+str(ins)+' & '+"%0.2f" % cpu_time+' & '+str(numb_iter)+' & 0 & 0 & '+str([len(S[i]) for i in range(m)])+' & '+str(numb_back) +' & ')
                    else:
                        ...
                        if sum(1 for s in ne if s>=1-10**-6) <m:
                            f_knapsack.write(" & & "+str(ins)+' & '+"%0.2f" % cpu_time+' & '+str(numb_iter)+' & 0 & 1 & '+str([len(S[i]) for i in range(m)])+' & '+str(numb_back) +' & ')
                            numb_mNE = numb_mNE+1
                        else:
            ...
            time_all = (time_all*1.)/aux
            total_size_0_PNS = (total_size_0_PNS*1.)/aux
            total_size_1_PNS = (total_size_1_PNS*1.)/aux
            total_size_2_PNS = (total_size_2_PNS*1.)/aux
            f_knapsack = open("KnapsackGeneralTable_only_PNS.txt",'a')
            if m>2:
